...
public class ProductsPartsController implements Initializable {
    ...
    public void afficherProduits() throws SQLException{
        ...
        for(int i = 0;i<pieces.size();i++){
            Label label = new Label(pieces.get(i).getNomProduit());
            label.setStyle("-fx-font-family: Quicksand;" +
                    "-fx-font-size: 30px;");
            String url = "file:"+getImageFullUrl(pieces.get(i).getImg_url());
            Image image = new Image(url);
            ImageView imageView = new ImageView(image);
            imageView = imageSets(imageView);
            Label label1 = new Label(String.valueOf(pieces.get(i).getPrix())+" TND");
