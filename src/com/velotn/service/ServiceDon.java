...
public class ServiceDon implements IService<Don> {
    ...
    public void ajouter(Don don) throws SQLException {
        PreparedStatement pre = con.prepareStatement("INSERT INTO `don` (`somme`, `date_don`) VALUES (?,?);");
    ...
    public Don search(Don don) throws SQLException {
        ...
        if(nb != 0){
            while(rs.next()){
                Don donNew = new Don(rs.getInt(1),rs.getDouble(2),(LocalDate) rs.getObject(3));
                return donNew;
            }
        }
        return null;
    ...
    public List<Don> sortByPrice() throws SQLException {
        ...
        while(rs.next()){
            ...
            Don donNew = new Don(id,somme,date);
            arr.add(donNew);
        }
        return arr;
    }
